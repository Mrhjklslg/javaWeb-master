...
public class UserDaoImpl implements UserDao {
    ...
    @Override
    public void addLzw(User user) {
        //1.定义sql
        String sql = "insert into user values(?,?,?,?,?,?,?,?)";
        //2.执行sql
        template.update(sql,user.getId(), user.getName(), user.getGender(), user.getAge(), user.getAddress(), user.getQq(), user.getEmail(),user.getPassword());
    }
